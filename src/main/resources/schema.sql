...
VALUES
  (1, 'ROLE_MANAGER'),
  (2, 'ROLE_EMPLOYEE'),
  (3, 'ROLE_CUSTOMER') ON DUPLICATE KEY
UPDATE id = id;
...
CREATE TABLE
  IF NOT EXISTS `customer_login_info` (
    `id` int NOT NULL AUTO_INCREMENT,
    `password` varchar(255) DEFAULT NULL,
...
CREATE TABLE
  IF NOT EXISTS `trigger_contract` (
...
CREATE TABLE
  IF NOT EXISTS `contract_settings` (
    `id` int unsigned NOT NULL AUTO_INCREMENT,
...
CREATE TABLE
  IF NOT EXISTS `google_drive_file` (
    `id` int unsigned NOT NULL AUTO_INCREMENT,
...
CREATE 
  OR REPLACE VIEW top_client as 
SELECT
  c.customer_id,
  c.name,
  COALESCE(tle.total_budget, 0) + COALESCE(tli.total_budget, 0) as total_depense
FROM
  customer c
  LEFT JOIN (
    SELECT
      customer_id,
      SUM(expense) as total_budget
    FROM
      trigger_lead
    GROUP BY
      customer_id
  ) tle ON tle.customer_id = c.customer_id
  LEFT JOIN (
    SELECT
      customer_id,
      SUM(expense) as total_budget
    ...
    GROUP BY
      customer_id
  ) tli ON tli.customer_id = c.customer_id
...
CREATE
  OR REPLACE VIEW montant_available_per_customer AS
SELECT
  c.customer_id as id,
  c.name as username,
...
FROM
  ...
  LEFT JOIN (
    SELECT
      customer_id,
      SUM(expense) AS total_depense_lead
    FROM
      trigger_lead
    GROUP BY
      customer_id
  ...
  LEFT JOIN (
    SELECT
      customer_id,
      SUM(expense) AS total_depense_ticket
    FROM
      trigger_ticket
    GROUP BY
      customer_id
